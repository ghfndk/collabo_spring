...
public class Order { // 주문과 관련된 Entity입니다.
    ...
    @ManyToOne(fetch = FetchType.LAZY) // To왼쪽은 클래스를 의미하고 To오른쪽은 member을 의미한다 (위 설명을 참조)
    @JoinColumn(name = "member_id", nullable = false)
    private Member member;

    // 통상적으로 우리가 주문을 할때, 여러 개의 `주문 상태`을 동시에 보여줍니다.
    // 하나의 주문에는 '주문 상품'을 여러개 담을 수 있습니다.
    // 주의) mappedBy 항목의 "order"는 OrderProduct에 들어 있는 Order 타입의 변수 명입니다.
