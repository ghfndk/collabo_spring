...
//    @GetMapping("/list") // 페이징 관련 파라미터를 사용하여 일부 상품 목록을 조회합니다.
...
//
//        // 현재 페이지는 pageNumber이고, 페이자당 보여줄 갯수 pageSize를 사용하여 Pageable 페이즈를 구현합니다.
//        // (Sort) 상품 번호가 큰 것부터 정렬합니다.
...
//    }
    ...
    public ResponseEntity<Page<Product>> listProducts(
        ...
        // Http 응답 코드 200과 함꼐 상품 정보를 JSON 형태로 반환해 줍니다.
    ...
    public ResponseEntity<?> insert(@RequestBody Product product) {
        ...
        // File.separator : 폴더 구분자를 의미하며, 리눅스는 /, 원도우는 \입니다.
        ...
        try {
            ...
            byte[] decodeImage = Base64.getDecoder().decode(imageData.split(",")[1]);
            fos.write(decodeImage); // 바이트 파일을 해당 이미지 경로에 복사하기
    ...
    // 프론트 앤드의 상품 수정 페이지이서 요청이 들어 왔습니다.
    ...
    public ResponseEntity<Product> getUpdate(@PathVariable Long id) {
        ...
        } else { // 해당 상품의 정보와 함꼐, 성공(200) 메시지를 반환합니다.
...
//        System.out.println(updatedProduct);
        ...
        if (findProduct.isEmpty()) { // 상품이 존재하지 않으면 404 응답 반환
            ...
            } catch (Exception err) { // 오류 발샐시 500 응답 코드 반환
